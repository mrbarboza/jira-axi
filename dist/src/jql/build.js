...
export function buildJql(options) {
    ...
    if (options.assignee)
        clauses.push(`assignee = ${quote(options.assignee)}`);
    if (options.project)
        clauses.push(`project = ${quote(options.project)}`);
    if (options.status)
        clauses.push(`status = ${quote(options.status)}`);
    if (options.label)
        clauses.push(`labels = ${quote(options.label)}`);
...
export function buildTextSearchJql(text, project) {
    const clauses = [`text ~ ${quote(text)}`];
    if (project)
        clauses.push(`project = ${quote(project)}`);
    return { jql: `${clauses.join(" AND ")} ORDER BY updated DESC`, source: "built" };
}
