...
export const SEARCH_HELP = `usage: nu-jira-axi search <text> [--project K]
examples:
  nu-jira-axi search "payment timeout"
  nu-jira-axi search "payment timeout" --project PROJ
`;
export async function searchCommand(args, site) {
    if (!site) {
        throw missingSiteError();
    }
    const text = getPositional(args, 0);
    if (!text) {
        throw new AxiError("usage: nu-jira-axi search <text> [--project K]", "VALIDATION_ERROR");
    }
